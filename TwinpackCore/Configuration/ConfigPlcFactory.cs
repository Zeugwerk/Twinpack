...
#pragma warning restore VSTHRD010 // Invoke single-threaded types on Main thread
        ...
        public static IEnumerable<(ConfigPlcProject Plc, PlcPublishMetadata Metadata)> PlcProjectsFromConfig(bool compiled, string target, string rootPath = ".", string cachePath = null)
        {
            ...
            {
                plc.FilePath = System.IO.Path.Combine(cachePath ?? Protocol.TwinpackServer.DefaultLibraryCachePath, target, $"{plc.Name}_{AutomationInterface.TwincatNumericVersion(AutomationInterface.NormalizedVersion(plc.Version))}.{suffix}");
